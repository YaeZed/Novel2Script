...
## 2026-06-07: 三幕边界和章节节拍在手测中出现语义漂移

### 现象
- 长文处理中，部分已经生成的场景曾从 `收束` 回到 `展开`。
- 灯塔样例里，`背面写着` 被识别成角色，角色表还出现过 `救船终于`。
- 第三章对照时，沈岚的总结台词被放到旧管理员提供十年前沉船信息之前。

### 根因
- partial 草稿每完成一章都会重新用当前场景数做三幕拆分，随着场景总数增长，幕归属会变化。
- 角色抽取的冒号标签和叙述动作规则过宽，把物件标签和句中片段当成角色证据。
- 真实模型会为了戏剧表达重排节拍，即使内容都来自原文，也会破坏对照页的阅读顺序。

### 用户影响
- 作者会误以为三幕结构不稳定，降低对结构规划的信任。
- 角色表混入物件标签会污染后续 prompt，增加假角色和错误对白。
- 对照页左右阅读顺序不一致，用户需要额外判断剧本是否漏写或改写了原文。

### 处理决策
- partial 草稿只使用单幕 `已处理部分`；最终完成后才显示开端、展开、收束。
- 角色抽取过滤物件/叙述标签，并要求中文动作证据从文本边界开始。
- 单章转换 prompt 明确要求节拍保持原文时间顺序；后端对真实模型 beats 做原文锚点排序兜底。

### 修复与验证
- 修复位置：
  - `backend/converter/services/script_assembler.py`
  - `backend/converter/services/character_extractor.py`
  - `backend/converter/services/llm_scene_converter.py`
  - `backend/converter/prompts.py`
- 回归测试：
  - partial 草稿只显示 `已处理部分`。
  - 角色表不包含 `背面写着`、`救船终于`。
  - 灯塔第三章节拍按原文顺序输出。
