...
- 剧本文件格式详见 [`docs/schema.md`](docs/schema.md)。后端/API 使用英文 key；对照页编辑区会显示中文字段，并在校验和解析前映射回内部字段。
...
- 对照页支持浅色/深色护眼阅读、阅读密度调整、场景标记和文本颜色标记。
- 对照页中文字段编辑表面与后端剧本 Schema 解耦，审阅标记只保存在当前浏览器。
